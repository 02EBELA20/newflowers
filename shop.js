...
const itemsPerPage = 8; // თითო გვერდზე 8 ქარდი (4x2 განლაგება)
let currentPage = 1;
...
function updateItems() {
    ...
    if (currentFilter !== 'all') {
        filteredItems = filteredItems.filter(item => {
            const categories = item.getAttribute('data-category').split(' '); // გავყოთ კატეგორიები სფეისით
            return categories.includes(currentFilter); // შევამოწმოთ, შეიცავს თუ არა მიმდინარე ფილტრს
        });
    }
